...
test_that("Constructor set only numeric data", {
  ...
  target <- data.frame(names=names[1], metrics1=m1[1], metrics2=m2[1], metrics3=m3[1])
  ...
  expect_equal(actual$all_metrics, data.frame(metrics1=m1, metrics2=m2))
  ## checking to be filtered non-numeric data
  expect_equal(actual$target_metrics, data.frame(metrics1=m1[1], metrics2=m2[1]))
